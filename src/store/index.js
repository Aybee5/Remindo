...
export default new Vuex.Store({
  ...
  actions: {
    ...
    checkUpcomingTime(context) {
      console.log("entering action")
      let upComingTime = context.getters.getUpcomingTodoTime
      upComingTime.forEach(todo => {
        let upComingTime = todo.fullTime
        if (moment(upComingTime).isSameOrBefore(moment())) {
          let itemIndex = context.state.upcoming.indexOf(todo)
          let item = context.state.upcoming.splice(itemIndex, 1)[0]
          context.state.completed.push(item)
